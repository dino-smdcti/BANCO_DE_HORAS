...
from src.domain.model import DailyPonto, User, UserRole

def test_daily_balance_calculation_8h_target():
    # User works 8h (480 mins) exactly, should have 0 balance
    user = User(email="test@test.com", password_hash="hash", role=UserRole.EMPLOYEE)
    ponto = DailyPonto(
        user_id=1,
...
def test_manager_time_tracking():
    ...
    user = User(email="mgr@test.com", password_hash="hash", role=UserRole.MANAGER)
    ponto = DailyPonto(
        user_id=2,
        entry_date=date(2026, 5, 4),
        arrival=time(10, 0),
